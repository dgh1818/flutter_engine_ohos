...
void checkPlatformEnvironment(String environment, Logger? logger) {
  ...
  if (environmentConfig == null) {
    throwToolExit('error:current platform environment $environment have not set');
  } else {
    logger?.printStatus('current platform environment $environment = $environmentConfig');
  }
}
...
String getHvigorwPath(String ohosRootPath, {bool checkMod = false}) {
  final String hvigorwPath = globals.fs.path.join(ohosRootPath, getHvigorwFile());
...
Future<void> ohpmInstall({
  ...
  processUtils.runSync(installCmd, workingDirectory: workingDirectory, throwOnError: true);
}

///hvigorw任务
Future<int> hvigorwTask(
  List<String> taskCommand, {
  required ProcessUtils processUtils,
...
void copyFlutterRuntime(
  ...
  final targetArchs = List<OhosArch>.of(ohosBuildInfo.targetArchs);
  for (var i = 0; i < targetArchs.length; i++) {
    ...
    if (i == 0) {
      ...
      final embeddingHarName = 'flutter_embedding_${buildMode.name}.har';
      _copyFile(originHarDir, desHarDir, embeddingHarName);
    }

    // Copy ${arch}_${buildMode.name}.har file
    final harName = '${arch.name}_${buildMode.name}.har';
...
String moduleNameWithFlavor(List<OhosModule> modules, String? flavor) {
  return modules
      .map(
        (OhosModule module) => OhosModule.fromModulePath(
          modulePath: module.srcPath,
          flavor: getFlavor(
            globals.fs.file(globals.fs.path.join(module.srcPath, 'build-profile.json5')),
            flavor,
          ),
        ),
      )
...
class OhosHvigorBuilder implements OhosBuilder {
  ...
  void printHowToConsumeHar({Logger? logger, BuildMode mode = BuildMode.release}) {
...
void _appendCommands(
  ...
  if (localEngineInfo != null) {
    ...
    command.addAll(<String>['-p', 'LOCAL_ENGINE=${localEngineInfo.localTargetName}']);
    command.addAll(<String>['-p', 'LOCAL_ENGINE_HOST=${localEngineInfo.localHostName}']);
    command.addAll(<String>[
      '-p',
    ...
    final Iterable<String> targetArchs = ohosBuildInfo.targetArchs.map(
      (OhosArch arch) => getPlatformNameForOhosArch(arch),
    );
    command.addAll(<String>['-p', 'TARGET_PLATFORM=${targetArchs.join(',')}']);
  }
  final Map<String, String> envConfig = ohosBuildInfo.buildInfo.toEnvironmentConfig();
