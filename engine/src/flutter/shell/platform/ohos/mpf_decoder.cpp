...
bool ParseGainmapHeadroomFromXmp(const uint8_t* data,
  ...
  while (search_start < end) {
    const uint8_t* found = std::search(search_start, end, kKey, kKey + kKeyLen);
    if (found == end) {
      return false;
    }
...
OH_PixelmapNative* CopyPixelmapToDmaRGBA8888(OH_PixelmapNative* src) {
  ...
  std::vector<uint8_t> src_buf(static_cast<size_t>(info.row_stride) *
                               info.height);
  size_t src_size = src_buf.size();
  Image_ErrorCode r =
      OH_PixelmapNative_ReadPixels(src, src_buf.data(), &src_size);
  if (r != IMAGE_SUCCESS) {
    FML_LOG(ERROR) << "[MPF] CopyPixelmapToDmaRGBA8888: ReadPixels failed:"
                   << r;
    return nullptr;
  }
  ...
  FML_LOG(ERROR) << "[MPF] CopyPixelmapToDmaRGBA8888: desired_stride="
                 << desired_stride;
  Image_ErrorCode stride_err = OH_PixelmapInitializationOptions_SetRowStride(
  ...
  if (e != IMAGE_SUCCESS || dst_pm == nullptr) {
    FML_LOG(ERROR)
        << "[MPF] CopyPixelmapToDmaRGBA8888: create DMA pixelmap failed:" << e;
    return nullptr;
  }
...
OH_PixelmapNative* ExpandSingleChannelToDmaRGBA8888(OH_PixelmapNative* src) {
  ...
  FML_LOG(ERROR) << "[MPF] ExpandSingleChannelToDmaRGBA8888: tight_stride="
                 << tight_stride << " src_stride=" << info.row_stride;
  if (dst_info.row_stride < tight_stride) {
    FML_LOG(ERROR)
  ...
  FML_LOG(ERROR) << "[MPF] ExpandSingleChannelToDmaRGBA8888: dst_stride="
                 << dst_info.row_stride;
  if (dst_info.row_stride % kRowStrideAlignment != 0) {
    FML_LOG(ERROR)
  ...
  if (wret != IMAGE_SUCCESS) {
    FML_LOG(ERROR)
        << "[MPF] ExpandSingleChannelToDmaRGBA8888: WritePixels failed:"
        << wret;
    OH_PixelmapNative_Release(dst_pm);
    return nullptr;
  }
...
bool ComposeGainmapToRGBA1010102(OH_PixelmapNative* sdr,
  ...
  if (sdr_info.width != gain_info.width ||
    ...
    FML_LOG(ERROR) << "[MPF] ComposeGainmap: size mismatch sdr="
                   ...
                   << " gainmap=" << gain_info.width << "x" << gain_info.height;
    return false;
  }
  const uint32_t tight_stride = sdr_info.width * kRgba8888Bytes;
  if (row_bytes < tight_stride) {
    FML_LOG(ERROR) << "[MPF] ComposeGainmap: row_bytes too small " << row_bytes
                   << " < " << tight_stride;
    return false;
  }
  ...
  const size_t tight_size = static_cast<size_t>(tight_stride) * sdr_info.height;
  ...
  const float safe_headroom = (std::isfinite(headroom) && headroom > 0.0f)
                                  ? headroom
...
bool ComposeMpfGainmap(const MpfGainmapComposeInput& input,
  ...
  *out = {};
  ...
  if (input.row_bytes < static_cast<size_t>(input.width) * kRgba8888Bytes) {
    FML_LOG(ERROR) << "[MPF] row_bytes too small:" << input.row_bytes
                   << " width:" << input.width;
    return false;
  }
  if (input.row_bytes % kRowStrideAlignment != 0) {
    FML_LOG(ERROR) << "[MPF] output row_bytes not 64-aligned: "
                   << input.row_bytes;
  }
      ...
      if (err == IMAGE_SUCCESS && dma_pm) {
        ...
        if (dma_handle.IsValid() &&
          ...
          FML_LOG(ERROR) << "[MPF] " << label
                         << " DMA decode success size:" << dma_handle.info.width
                         << "x" << dma_handle.info.height
  ...
  if (ComposeGainmapToRGBA1010102(base_pixelmap.pixelmap,
                                  gainmap_pixelmap.pixelmap,
                                  static_cast<uint8_t*>(input.pixels),
                                  input.row_bytes, input.headroom)) {
  ...
  auto LogPixelmapInfo = [&](const char* label,
    ...
    if (pixelmap.info.row_stride % kRowStrideAlignment != 0) {
      FML_LOG(ERROR) << "[MPF] " << label << " pixelmap stride not 64-aligned: "
                     << pixelmap.info.row_stride;
    }
    OH_NativeColorSpaceManager* color_space = nullptr;
    int color_space_name = 0;
    Image_ErrorCode cs_ret =
        OH_PixelmapNative_GetColorSpaceNative(pixelmap.pixelmap, &color_space);
  ...
  OH_PixelmapInitializationOptions_SetWidth(options,
                                            static_cast<uint32_t>(input.width));
  OH_PixelmapInitializationOptions_SetHeight(
      options, static_cast<uint32_t>(input.height));
  OH_PixelmapInitializationOptions_SetPixelFormat(options,
  ...
  auto SetHdrMetadataType = [&](const char* label, OH_PixelmapNative* pixelmap,
  ...
  }
  {
    ...
    uint32_t byte_count = 0;
    Image_ErrorCode byte_ret =
        OH_PixelmapNative_GetByteCount(gainmap_pixelmap.pixelmap, &byte_count);
    FML_LOG(ERROR) << "[MPF] gainmap GetByteCount: " << byte_ret
                   << " bytes:" << byte_count;
  }
  {
    OH_NativeBuffer* native_buffer = nullptr;
    Image_ErrorCode dma_ret =
        OH_PixelmapNative_GetNativeBuffer(hdr_handle.pixelmap, &native_buffer);
  ...
  bool compose_supported = OH_ImageProcessing_IsCompositionSupported(
      &src_info, &gain_info, &dst_info);
  FML_LOG(ERROR) << "[MPF] Compose support: " << compose_supported
  ...
  proc_ret = OH_ImageProcessing_Compose(instance, base_pixelmap.pixelmap,
                                        gainmap_pixelmap.pixelmap,
                                        hdr_handle.pixelmap);
  if (proc_ret != IMAGE_PROCESSING_SUCCESS) {
