 ...
 */
void OHOSUtils::SerializeString(const std::string& str,
 ...
 */
std::vector<uint8_t> OHOSUtils::SerializeStringIntMap(
    const std::map<std::string, int32_t>& mp) {
  std::vector<uint8_t> buffer;

  // Store the number of elements in the map
  uint32_t mapSize = mp.size();
  buffer.insert(buffer.end(), reinterpret_cast<uint8_t*>(&mapSize),
  ...
  for (const auto& it : mp) {
    ...
    buffer.insert(
        buffer.end(), reinterpret_cast<const uint8_t*>(&it.second),
