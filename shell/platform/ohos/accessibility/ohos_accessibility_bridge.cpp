...
#define ARKUI_ACCESSIBILITY_CALL_CHECK(X)                                      \
  do {                                                                         \
    ...
    if (RET != ARKUI_SUCCEED_CODE) {                                           \
      LOGE("Failed function %{public}s call, error code:%{public}d", #X, RET); \
    }                                                                          \
  } while (false)
...
void OhosAccessibilityBridge::OnOhosAccessibilityStateChange(
  ...
  if (ohosAccessibilityEnabled) {
    nativeAccessibilityChannel_->OnOhosAccessibilityEnabled(
        native_shell_holder_id_);
  } else {
    accessibilityFeatures_->SetAccessibleNavigation(false,
                                                    native_shell_holder_id_);
    nativeAccessibilityChannel_->OnOhosAccessibilityDisabled(
        native_shell_holder_id_);
  }
}
...
void OhosAccessibilityBridge::UpdateSemantics(
  ...
  for (auto& item : update) {
    ...
    for (int32_t i = 0; i < childNodeCount; i++) {
      g_parentChildIdVec.emplace_back(
          std::make_pair(nodeEx.id, nodeEx.childrenInTraversalOrder[i]));
    }
  }
  // 页面内容更新事件
  Flutter_SendAccessibilityAsyncEvent(
  ...
  for (const auto& nodeEx : updatedFlutterNodes) {
    ...
    if (HasScrolled(nodeEx)) {
      ...
      FlutterNodeToElementInfoById(_elementInfo,
                                   static_cast<int64_t>(nodeEx.id));
    ...
    if (nodeEx.HasFlag(FLAGS_::kIsLiveRegion) && HasChangedLabel(nodeEx)) {
      FML_DLOG(INFO) << "UpdateSemantics -> page content update, nodeEx.id="
                     << nodeEx.id;
      Flutter_SendAccessibilityAsyncEvent(
          static_cast<int64_t>(nodeEx.id),
          ArkUI_AccessibilityEventType::
  ...
  if (node.scrollChildren > 0) {
    ...
    ARKUI_ACCESSIBILITY_CALL_CHECK(
        OH_ArkUI_AccessibilityElementInfoSetCurrentItemIndex(
            elementInfoFromList, currItemIndex));
    ...
    if (!node.childrenInHitTestOrder.size()) {
      FML_DLOG(WARNING)
          << "FlutterScrollExecution -> Had scrollChildren but no "
...
void OhosAccessibilityBridge::RequestFocusWhenPageUpdate(
  ...
  ARKUI_ACCESSIBILITY_CALL_CHECK(OH_ArkUI_AccessibilityEventSetEventType(
      reqFocusEventInfo,
      ArkUI_AccessibilityEventType::
          ARKUI_ACCESSIBILITY_NATIVE_EVENT_TYPE_REQUEST_ACCESSIBILITY_FOCUS));

  ARKUI_ACCESSIBILITY_CALL_CHECK(OH_ArkUI_AccessibilityEventSetRequestFocusId(
      reqFocusEventInfo, requestFocusId));
  ARKUI_ACCESSIBILITY_CALL_CHECK(OH_ArkUI_AccessibilityEventSetElementInfo(
      reqFocusEventInfo, elementInfo));
 ...
 */
void OhosAccessibilityBridge::SetAbsoluteScreenRect(
...
AbsoluteRect OhosAccessibilityBridge::GetAbsoluteScreenRect(
    SemanticsNodeExtent& flutterNode) {
...
void OhosAccessibilityBridge::FlutterRelativeRectToScreenRect(
  ...
  if (!g_flutterSemanticsTree.count(parentId)) {
    LOGE(
        "FlutterRelativeRectToScreenRect: GetFlutterSemanticsNode "
        "id=%{public}d null",
        parentId);
  }

  // 获取当前flutter节点的父节点的绝对坐标
  auto [realParentLeft, realParentTop, realParentRight, realParentBottom] =
      GetAbsoluteScreenRect(parentNode);
  ...
  if (_kMScaleX > 1 && _kMScaleY > 1) {
    ...
    const bool IS_OVER_SCREEN_AREA =
        newLeft < realParentLeft || newTop < realParentTop ||
        newRight > realParentRight || newBottom > realParentBottom ||
        newLeft >= newRight || newTop >= newBottom || newRight > rootWidth ||
        newBottom > rootHeight;

    if (IS_OVER_SCREEN_AREA) {
      FML_DLOG(WARNING)
          << "RelativeRectToScreenRect -> childRect exceeds parentRect {Id: "
...
void OhosAccessibilityBridge::FlutterNodeToElementInfoById(
  ...
  if (elementId == 0 || elementId == -1) {
    ...
    if (flutterNode.isNull) {
      LOGE(
          "FlutterNodeToElementInfoById: GetFlutterSemanticsNode "
          "id=%{public}ld null",
          elementId);
    }
    ...
    ARKUI_ACCESSIBILITY_CALL_CHECK(
        OH_ArkUI_AccessibilityElementInfoSetScreenRect(elementInfoFromList,
                                                       &rect));
    ...
    // 设置root节点的action类型
    FlutterSetElementInfoOperationActions(elementInfoFromList,
                                          OTHER_WIDGET_NAME);
    ...
    ARKUI_ACCESSIBILITY_CALL_CHECK(OH_ArkUI_AccessibilityElementInfoSetParentId(
    ...
    ARKUI_ACCESSIBILITY_CALL_CHECK(
        OH_ArkUI_AccessibilityElementInfoSetClickable(elementInfoFromList,
                                                      true));
    ARKUI_ACCESSIBILITY_CALL_CHECK(
        OH_ArkUI_AccessibilityElementInfoSetFocusable(elementInfoFromList,
                                                      true));
    ARKUI_ACCESSIBILITY_CALL_CHECK(
    ...
    ARKUI_ACCESSIBILITY_CALL_CHECK(
        OH_ArkUI_AccessibilityElementInfoSetComponentType(elementInfoFromList,
                                                          "root"));
    ARKUI_ACCESSIBILITY_CALL_CHECK(OH_ArkUI_AccessibilityElementInfoSetContents(
        elementInfoFromList, flutterNode.label.c_str()));
  } else {
...
void OhosAccessibilityBridge::FlutterSetElementInfoProperties(
  ...
  if (flutterNode.isNull) {
    LOGE(
        "FlutterSetElementInfoProperties: GetFlutterSemanticsNode "
        "id=%{public}ld null",
        elementId);
  }
  ...
  ARKUI_ACCESSIBILITY_CALL_CHECK(OH_ArkUI_AccessibilityElementInfoSetElementId(
      elementInfoFromList, flutterNode.id));
  ...
  ARKUI_ACCESSIBILITY_CALL_CHECK(OH_ArkUI_AccessibilityElementInfoSetScreenRect(
  ...
  if (IsTextField(flutterNode)) {
    ...
    std::string widgeType = flutterNode.HasFlag(FLAGS_::kIsMultiline)
                                ? EDIT_MULTILINE_TEXT_WIDGET_NAME
                                : EDIT_TEXT_WIDGET_NAME;
  ...
  // 设置无障碍content文本
  ARKUI_ACCESSIBILITY_CALL_CHECK(OH_ArkUI_AccessibilityElementInfoSetContents(
      elementInfoFromList, text.c_str()));
  // 设置hint提示文本
  std::string hint = flutterNode.hint;
  ARKUI_ACCESSIBILITY_CALL_CHECK(OH_ArkUI_AccessibilityElementInfoSetHintText(
      elementInfoFromList, hint.c_str()));
  ...
  if (childCount > 0) {
    ...
    for (int32_t i = 0; i < childCount; i++) {
      childNodeIds[i] = static_cast<int64_t>(childrenIdsVec[i]);
      FML_DLOG(INFO) << "FlutterNodeToElementInfoById -> elementid="
                     << elementId << " childCount=" << childCount
  ...
  if (IsNodePassword(flutterNode)) {
    ARKUI_ACCESSIBILITY_CALL_CHECK(
        OH_ArkUI_AccessibilityElementInfoSetIsPassword(elementInfoFromList,
                                                       true));
    FML_DLOG(INFO) << "flutterNode.id=" << flutterNode.id
  ...
  if (IsNodeCheckable(flutterNode)) {
    ARKUI_ACCESSIBILITY_CALL_CHECK(
        OH_ArkUI_AccessibilityElementInfoSetCheckable(elementInfoFromList,
                                                      true));
    FML_DLOG(INFO) << "flutterNode.id=" << flutterNode.id
  ...
  if (IsNodeScrollable(flutterNode)) {
    ARKUI_ACCESSIBILITY_CALL_CHECK(
        OH_ArkUI_AccessibilityElementInfoSetScrollable(elementInfoFromList,
                                                       true));
    FML_DLOG(INFO) << "flutterNode.id=" << flutterNode.id
  ...
  // 判断当前节点组件是否可编辑（文本输入框）
  if (IsTextField(flutterNode)) {
    ARKUI_ACCESSIBILITY_CALL_CHECK(OH_ArkUI_AccessibilityElementInfoSetEditable(
        elementInfoFromList, true));
    FML_DLOG(INFO) << "flutterNode.id=" << flutterNode.id
 ...
 */
std::vector<int64_t> OhosAccessibilityBridge::GetLevelOrderTraversalTree(
    int32_t rootId) {
...
void OhosAccessibilityBridge::BuildArkUISemanticsTree(
  ...
  for (int64_t i = 1; i < elementInfoCount; i++) {
    ...
    if (g_flutterSemanticsTree.count(newNode.id)) {
      LOGE(
          "BuildArkUISemanticsTree: GetFlutterSemanticsNode id=%{public}ld "
          "null",
          levelOrderId);
    }
    // 当节点为隐藏状态时，自动规避
...
int32_t OhosAccessibilityBridge::FindAccessibilityNodeInfosById(
  ...
  if (!g_flutterSemanticsTree.count(flutterNode.id)) {
    LOGE(
        "FindAccessibilityNodeInfosById: GetFlutterSemanticsNode "
        "id=%{public}ld is null",
        elementId);
  }

  // 开启无障碍导航功能
  if (elementId == -1 || elementId == 0) {
    accessibilityFeatures_->SetAccessibleNavigation(true,
                                                    native_shell_holder_id_);
  }
...
void OhosAccessibilityBridge::DispatchSemanticsAction(
  ...
  nativeAccessibilityChannel_->DispatchSemanticsAction(
      native_shell_holder_id_, id, action, fml::MallocMapping());
}

/**
 * flutter按钮节点双击跳转新页面时，发送页面更新事件
 */
void OhosAccessibilityBridge::DoubleClickRouteToNewPage(
    SemanticsNodeExtent node) {
...
void OhosAccessibilityBridge::PerformLongClickAction(
  ...
  FML_DLOG(INFO) << "ExecuteAccessibilityAction -> action: longclick(" << action
                 << ")" << " event: longclick(" << longClickEventType << ")";
...
void OhosAccessibilityBridge::PerformScrollUpAction(
  ...
  if (currComponetType.find(SCROLL_WIDGET_NAME) != std::string::npos) {
    ...
    FML_DLOG(INFO) << "ExecuteAccessibilityAction -> action: scroll forward("
                   << action << ")" << " event: scroll forward("
                   << scrollEventType1 << ")";
  }
}
...
void OhosAccessibilityBridge::PerformClipboardAction(
    int64_t elementId,
    ArkUI_Accessibility_ActionType action) {
  if (action == ArkUI_Accessibility_ActionType::
                    ARKUI_ACCESSIBILITY_NATIVE_ACTION_TYPE_COPY) {
    ...
    DispatchSemanticsAction(static_cast<int32_t>(elementId), ACTIONS_::kCopy,
                            {});
  } else if (action == ArkUI_Accessibility_ActionType::
                           ARKUI_ACCESSIBILITY_NATIVE_ACTION_TYPE_PASTE) {
...
void OhosAccessibilityBridge::PerformInvalidAction(
    ...
    SemanticsNodeExtent flutterNode) {
  /** Invalid event. 0 */
  ArkUI_AccessibilityEventType invalidEventType = ArkUI_AccessibilityEventType::
      ARKUI_ACCESSIBILITY_NATIVE_EVENT_TYPE_INVALID;
  Flutter_SendAccessibilityAsyncEvent(elementId, invalidEventType);
  FML_DLOG(ERROR) << "ExecuteAccessibilityAction -> action: invalid(" << action
                  << ")" << " event: innvalid(" << invalidEventType << ")";
}
/**
 * 设置输入框文本
 */
void OhosAccessibilityBridge::PerformSetText(
  ...
  char* newText;
  ARKUI_ACCESSIBILITY_CALL_CHECK(OH_ArkUI_FindAccessibilityActionArgumentByKey(
      actionArguments, ARKUI_ACTION_ARG_SET_TEXT, &newText));
  if (newText == nullptr) {
    LOGE(
  ...
  LOGI(
      "ExecuteAccessibilityAction -> action: set text(%{public}d), "
      "newText=%{public}s",
      action, newText);
}
...
void OhosAccessibilityBridge::PerformSelectText(
  ...
  bool hasSelected = actionArguments != nullptr && textSelectBase != nullptr &&
  ...
  if (hasSelected) {
    ...
    selectionMap.insert(
        {ARKUI_ACTION_ARG_SELECT_TEXT_START, flutterNode.textSelectionBase});
    selectionMap.insert(
        {ARKUI_ACTION_ARG_SELECT_TEXT_END, flutterNode.textSelectionExtent});
  }
  // serialize map<string, int32_t> to byte vector
  std::vector<uint8_t> encodedData =
      OHOSUtils::SerializeStringIntMap(selectionMap);
  DispatchSemanticsAction(
      flutterNode.id, flutterSelectTextAction,
      fml::MallocMapping::Copy(encodedData.data(),
                               encodedData.size() * sizeof(uint8_t)));
}
...
void OhosAccessibilityBridge::PerformSetCursorPosition(
    ...
    ArkUI_AccessibilityActionArguments* actionArguments) {
  FML_DLOG(INFO)
      << "ExecuteAccessibilityAction -> action: set cursor position (" << action
      << ")";
  return;
}
...
void OhosAccessibilityBridge::PerformCustomAction(
    ...
    ArkUI_Accessibility_ActionType action,
    ArkUI_AccessibilityActionArguments* actionArguments) {
  FML_DLOG(INFO) << "ExecuteAccessibilityAction -> action: custom action ("
                 << action << ")";
...
int32_t OhosAccessibilityBridge::ExecuteAccessibilityAction(
  ...
  if (!g_flutterSemanticsTree.count(flutterNode.id)) {
    LOGE(
        "ExecuteAccessibilityAction: GetFlutterSemanticsNode id=%{public}ld is "
        "null",
        elementId);
  }
  /**
 ...
 */
bool OhosAccessibilityBridge::IsNodeClickable(SemanticsNodeExtent flutterNode) {
  return flutterNode.HasAction(ACTIONS_::kTap);
}
/**
 * 判断当前flutter节点组件是否可显示
 */
bool OhosAccessibilityBridge::IsNodeVisible(SemanticsNodeExtent flutterNode) {
  return !flutterNode.HasFlag(FLAGS_::kIsHidden);
}
/**
 * 判断当前flutter节点组件是否具备checkable属性
 */
bool OhosAccessibilityBridge::IsNodeCheckable(SemanticsNodeExtent flutterNode) {
 ...
 */
bool OhosAccessibilityBridge::IsNodeChecked(SemanticsNodeExtent flutterNode) {
 ...
 * 判断当前flutter节点组件是否选中
 */
bool OhosAccessibilityBridge::IsNodeSelected(SemanticsNodeExtent flutterNode) {
  return flutterNode.HasFlag(FLAGS_::kIsSelected);
}
/**
 * 判断当前flutter节点组件是否为密码输入框
 */
bool OhosAccessibilityBridge::IsNodePassword(SemanticsNodeExtent flutterNode) {
  return flutterNode.HasFlag(FLAGS_::kIsTextField) &&
 ...
 */
bool OhosAccessibilityBridge::IsNodeEnabled(SemanticsNodeExtent flutterNode) {
 ...
 * 判断当前节点是否改变标签文本
 */
bool OhosAccessibilityBridge::HasChangedLabel(
    const SemanticsNodeExtent& flutterNode) {
  if (flutterNode.label.empty() && flutterNode.previousLabel.empty()) {
    return false;
  }
  return flutterNode.label.empty() || flutterNode.previousLabel.empty() ||
         flutterNode.label != flutterNode.previousLabel;
}
/**
 * 判断当前语义节点是否可获焦
 */
bool OhosAccessibilityBridge::IsNodeFocusable(const SemanticsNodeExtent& node) {
 ...
 * 判断当前flutter节点是否获焦
 */
bool OhosAccessibilityBridge::IsNodeFocused(
    const SemanticsNodeExtent& flutterNode) {
...
void OhosAccessibilityBridge::RemoveSemanticsNode(
  ...
  for (auto it = g_parentChildIdVec.begin(); it != g_parentChildIdVec.end();
       it++) {
...
void OhosAccessibilityBridge::GetSemanticsNodeDebugInfo(
  ...
  FML_DLOG(INFO) << "node.previousTextSelectionBase="
                 << node.previousTextSelectionBase;
  FML_DLOG(INFO) << "node.textSelectionExtent=" << node.textSelectionExtent;
  FML_DLOG(INFO) << "node.previousTextSelectionExtent="
                 << node.previousTextSelectionExtent;
  ...
  FML_DLOG(INFO) << "node.previousScrollPosition="
                 << node.previousScrollPosition;
  FML_DLOG(INFO) << "node.scrollIndex=" << node.scrollIndex;
  FML_DLOG(INFO) << "node.scrollExtentMax=" << node.scrollExtentMax;
  FML_DLOG(INFO) << "node.previousScrollExtentMax="
                 << node.previousScrollExtentMax;
  FML_DLOG(INFO) << "node.scrollExtentMin=" << node.scrollExtentMin;
  FML_DLOG(INFO) << "node.previousScrollExtentMin="
                 << node.previousScrollExtentMin;
